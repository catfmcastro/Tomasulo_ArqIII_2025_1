...
// imprime estado dos registradores em uma linha
void print_registers() {
    printf("Registers: ");
    for (int i = 0; i < reg_size; i++) {
        printf("r%d=%d", i, regs.values[i]);
        if (i < reg_size - 1) {
            printf(" ");
        }
    }
...
void execute() {
    for (int i = 0; i < rs_size; i++) {
        ...
        if (r->busy && r->qj == -1 && r->qk == -1) {
            // operandos prontos
            int result = 0;
            switch (r->op) {
                case ADD: result = r->vj + r->vk;
                    break;
                case SUB: result = r->vj - r->vk;
                    break;
                case MUL: result = r->vj * r->vk;
                    break;
                case DIV: result = (r->vk ? r->vj / r->vk : 0);
                    break;
                case LI: result = r->vj;
                    break;
                default: break;
...
void write_result() {
    ...
    // ai tenta commit no head do ROB
...
int main() {
    ...
    while (program[control.pc].op != HALT) {
        // imprime estado dos registradores antes do clock
    ...
    // imprime estado final dos registradores
    printf("----------- Final State:  ------------ \n");
    print_registers();
